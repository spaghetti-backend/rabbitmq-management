...
class AsyncQueuesAPI(BaseAPI):
    ...
    async def by_vhost(self, vhost: str) -> list[dict]:
        ...
        return await self._http_client.get(Paths.queues.by_vhost(vhost=vhost))

    async def detail(self, vhost: str, queue: str) -> dict:
        """Get details of a specific queue."""
        return await self._http_client.get(
            Paths.queues.detail(vhost=vhost, queue=queue)
        )

    async def set(self, vhost: str, queue: str, value: dict) -> dict:
        """
    ...
    async def delete(self, vhost: str, queue: str) -> dict:
        """Delete a specific queue."""
        return await self._http_client.delete(
            Paths.queues.detail(vhost=vhost, queue=queue)
        )

    async def bindings(self, vhost: str, queue: str) -> list[dict]:
        """List all bindings for a specific queue."""
        return await self._http_client.get(
            Paths.queues.bindings(vhost=vhost, queue=queue)
        )

    async def purge(self, vhost: str, queue: str) -> dict:
        """Remove all messages from the queue without deleting the queue itself."""
        return await self._http_client.delete(
            Paths.queues.contents(vhost=vhost, queue=queue)
        )

    async def actions(self, vhost: str, queue: str, value: dict) -> dict:
        """
    ...
    async def messages(self, vhost: str, queue: str, value: dict) -> list[dict]:
        """

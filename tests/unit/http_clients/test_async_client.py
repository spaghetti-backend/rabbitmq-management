...
async def test_get_succuss(
    async_http_client: client.AsyncHTTPClient, api_mock: MockRouter
):
...
async def test_connect_error_wraps_with_rmq_network_error(
    ...
    with pytest.raises(RMQNetworkError, match="Network connection failed"):
        await async_http_client.post("users/bulk-delete", payload={"users": ["test"]})

...
async def test_get_all_queues(
    ...
    response = await async_management_api.queues.all(
...
async def test_get_queue_detail(
    ...
    response = await async_management_api.queues.detail(vhost="/", queue="test")

    assert response.get("name") == "test"


async def test_set_queue(
    async_management_api: api.AsyncRMQManagementAPI, api_mock: MockRouter
):
    mock_json = {
        "auto_delete": False,
...
async def test_get_message_from_queue(
    ...
    response = await async_management_api.queues.messages(
